...
const DetailScreen = ({ route, navigation }) => {
    ...
    const { setFavouriteList } = useCustomHook()
    ...
    const addOnFavourite = () => {
        ...
        setFavouriteList((list) => [...list, { ...detail, quantity: counter }])
        putData('menu', id, { id: detail.id, imageUrl: detail.imageUrl, name: detail.name, include: detail.include, price: detail.price, description: detail.description, category: detail.category, rating: detail.rating, isOnCart: detail.isOnCart, isOnFavourite: true })
    }

    const removeOnFavourite = () => {
        deleteData('favourite', id)
        getDataById('menu', id, setDetail)
        putData('menu', id, { id: detail.id, imageUrl: detail.imageUrl, name: detail.name, include: detail.include, price: detail.price, description: detail.description, category: detail.category, rating: detail.rating, isOnCart: detail.isOnCart, isOnFavourite: false })
    }
        ...
        <View className={'flex-1 bg-white'}>
            <View className={'flex-[0.6] bg-slate-500 rounded-b-3xl'} >
                ...
                <View className={"flex-row justify-between items-center py-1 absolute top-1 w-[100%] p-3"}>
                    ...
                    <TouchableOpacity onPress={!detail?.isOnFavourite ? addOnFavourite : removeOnFavourite ?? addOnFavourite}>
                        <BlurView tint='dark' intensity={60} className={'rounded-lg overflow-hidden p-2'}>
                            <Icons.HeartIcon size={25} color={detail?.isOnFavourite ? 'red' : 'white' ?? 'white'} />
                        </BlurView>
                    </TouchableOpacity>
                </View>
